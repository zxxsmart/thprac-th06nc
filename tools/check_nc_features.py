...
report=dict(guards=rows,resources=resources,lock=[locked1,locked2],recordNumber=number)
Path('build/nc-features.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
print('resources, rank and time lock PASS',flush=True)
p.key(pid,27);time.sleep(.4);p.key(pid,81);time.sleep(2)
print('Native replay-save question opened; select a slot and confirm the name to save.')
